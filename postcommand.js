        ...
        client.on('interactionCreate', async (interaction3) => {
          ...
          try {
            if (interaction3.customId === 'approve_button') {
              ...
              const channel = await client.channels.fetch(channelId);
              postedMessage = await channel.send({ content: `<@${memberInteractor}>`, embeds: [embed] });
              ...
              try {
                ...
                interaction.user.send({ content: `:tada: Your post was successfully approved, Congratulations! :tada:`, embeds: [embed] });
              ...
              interaction3.reply(`Please enter the valid reason why you are declining this post`).then(() => {
                ...
                collector.on('collect', collected => {
                  ...
                  interaction.user.send({ content: `<a:Xmark:1106658889656709202> <@${memberInteractor}> Unfortunately your post in Bloxian Devs has been declined for the following reason: \`${reason}\``, embeds: [embed] });
                });
              });
            }
          } catch (error) {
